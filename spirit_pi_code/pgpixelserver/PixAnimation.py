...
# direction for auto animations. 0=move rearward 1= move frontward
...
def selectAnimation(socket_data):
    ...
    try:
        ...
        # malformed packet; keep the previous animation_mode and log the input
        print("PixAnimation: ignoring malformed packet:", socket_data)
    ...
    if animation_mode == 0:
        ...
        if new_data:  # update only if new_data flag is set
            ...
            animation_end_index = (
                PIXEL_LEFTW_END - PIXEL_LEFTW_START) + over_under
            new_data = 0  # clear new_data flag
        ...
        if new_data:  # update only if new_data flag is set
            ...
            animation_start_index = 0 - over_under
            animation_end_index = (
                PIXEL_RIGHTW_END - PIXEL_RIGHTW_START) + over_under
            new_data = 0  # clear new_data flag
        time.sleep(interval0 / 1000)
        update_pixel_index(animation_direction)
...
def wing_fade(hue, brightness, falloff, index_base, streak_mode, pix_first, pix_last):
    ...
    # int(socket_data[4]) + PIXEL_RIGHTW_START
    index_base = index_base + pix_first
    ...
    for x in range(1, 12):
        ...
        if streak_mode == 0 or streak_mode == 1:  # streak reaward in modes 0 or 1
            # if (down_pix >= 15):
            if (down_pix >= pix_first) and (down_pix <= pix_last):
...
def right_wing_fadedown_bak(socket_data):
    ...
    if new_data:
        ...
        for x in range(1, 12):
            ...
            if streak_mode == 0 or streak_mode == 1:  # streak reaward in modes 0 or 1
                # if (down_pix >= 15):
                if (down_pix >= PIXEL_RIGHTW_START) and (down_pix <= PIXEL_RIGHTW_END):
                    strip.setPixelColorRGB(down_pix, r, g, b)
                    # print "down:", down_pix
            else:
                # if (down_pix >= 15):
...
def getColorWheel(h, bright):
    ...
    b = 0
    s = 1
    # turn bright into float, otherwise python assumes int and rounds it
    bright = float(bright)
    v = bright / 255
    i = int(h/60.0)
    f = h/60.0 - i
